...
type path = string;
type interpret = (path, config, ?string) => config;
...
class ConfigParser extends Parser {
  ...
  body = this.RULE('body', (map: section, inclusion: boolean = false, condition: ?string = null): void => {
    this.MANY(() => {
      ...
      if (inclusion && key.image === 'path') {
        this._interpret(valueString, this._map, condition);
      }
    });
  });
