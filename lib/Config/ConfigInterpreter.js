...
class ConfigInterpreter {
  ...
  interpret (path: path, inclusion: boolean = true, limit: number = 10): config {
    // an inclusion limit of 10 replicates git behaviour
    // DAG like inclusions are allowed
    return this._interpret(path, new Map, inclusion, limit);
  }
      ...
      if (inclusion) {
        ...
        parser = new ConfigParser(
          (newPath, map, newLimit, newCondition) => {
            ...
            return this._interpret(newPath, map, true, newLimit, newCondition);
          },
          limit
        );
      } else {
